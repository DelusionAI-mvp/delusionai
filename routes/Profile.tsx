...
const generateDynamicStoryFromProfile = (profile: any): string => {
  const ageGroup = profile.age ? `As an individual in the ${Array.isArray(profile.age) ? profile.age[0] : profile.age} age bracket, ` : "Currently, ";
  
  const situations = profile.currentSituation || [];
  ...
  const struggles = profile.emotionalProfile?.moodKeywords || [];
  ...
  const whyHere = profile.whyJoined || [];
  ...
  if (sentences.length === 0) {
    return "Deeply reflecting, exploring support avenues, and conversing with Maya to unfold a unique soul narrative. Syncing with matching models in real-time.";
  }
...
export default function ProfilePage() {
  ...
  React.useEffect(() => {
    if (profile?.userMemorySummary) {
      setEditedStory(profile.userMemorySummary);
    } else if (profile) {
      setEditedStory(generateDynamicStoryFromProfile(profile));
    }
  }, [profile]);
  ...
  const handleSaveStory = async () => {
    ...
    try {
      ...
      await updateDoc(userRef, { userMemorySummary: editedStory });
      setIsEditingStory(false);
      setUploadStatus("Story Saved!");
    ...
    <div className="page-base global-container section-gap pt-4 md:pt-6">
      <div className="flex flex-col md:flex-row items-center gap-12 md:gap-16 cred-elevation p-10 sm:p-12 md:p-20 relative overflow-hidden backdrop-blur-xl border-brand-primary/10">
        ...
        <div className="text-center md:text-left space-y-4 sm:space-y-6 md:space-y-8 flex-1 w-full overflow-hidden">
          ...
          {/* Real-time Journey Narrative summarized dynamically by Maya / Editable by User */}
          {isEditingStory ? (
            <div className="cred-inset p-5 md:p-6 bg-white/[0.01] border-brand-primary/10 max-w-2.5xl space-y-3 text-left">
              <div className="flex items-center justify-between">
                <p className="text-[9px] font-black uppercase tracking-[0.3em] text-brand-primary/60">My Story (Editing)</p>
                <div className="flex gap-2">
                  <button 
                    onClick={() => setIsEditingStory(false)}
                    className="px-2 py-1 border border-brand-primary/20 text-[8px] font-mono font-black uppercase tracking-widest text-[#B42D46] hover:bg-[#B42D46]/10 rounded cursor-pointer animate-pulse"
                  >
                    Cancel
                  </button>
                  <button 
                    onClick={handleSaveStory}
                    className="px-2 py-1 border border-brand-accent/30 text-[8px] font-mono font-black uppercase tracking-widest text-[#DAA520] hover:bg-[#DAA520]/10 rounded cursor-pointer animate-pulse"
                  >
                    Save
                  </button>
                </div>
              ...
              <textarea
                ...
                placeholder="Write your soul journey here..."
                className="w-full h-24 p-4 bg-bg-base/65 border border-brand-primary/10 text-sm font-sans font-light leading-[1.8] text-text-base focus:outline-none focus:border-brand-primary/30 rounded-xl resize-none"
                maxLength={400}
              />
            </div>
          ) : (
            <div className="cred-inset p-5 md:p-6 bg-white/[0.01] border-brand-primary/10 max-w-2.5xl space-y-2 text-left relative group">
              <div className="flex items-center justify-between">
                <p className="text-[9px] font-black uppercase tracking-[0.3em] text-brand-primary/60">My Story</p>
                <button 
                  onClick={() => {
                    setEditedStory(profile.userMemorySummary || generateDynamicStoryFromProfile(profile));
                    setIsEditingStory(true);
                  }}
                  className="px-2 py-1 border border-brand-primary/15 text-[8px] font-mono font-black uppercase tracking-widest text-brand-primary rounded bg-brand-primary/[0.02] hover:bg-brand-primary/10 transition-colors cursor-pointer"
                >
                  Edit Story
                </button>
              </div>
              <p className="font-sans font-light text-sm sm:text-base text-text-muted leading-[1.8]">
                {profile.userMemorySummary || generateDynamicStoryFromProfile(profile)}
              </p>
            </div>
          )}
