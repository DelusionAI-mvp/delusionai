  ...
  return (
    <div className="min-h-screen flex flex-col bg-transparent text-text-base selection:bg-brand-primary selection:text-white relative">
      ...
      {(showNav || showPublicNav) && (
        <nav className={`fixed top-0 left-0 right-0 z-50 py-4 sm:py-6 border-b-4 border-brand-primary/20 bg-bg-base/80 backdrop-blur-md transition-all duration-300 ${showHeader ? 'translate-y-0' : '-translate-y-full'}`}>
              ...
              {showNav ? (
                <div className="flex items-center gap-2 sm:gap-4">
                  <div className="hidden md:flex items-center gap-1 lg:gap-2">
                    <button onClick={() => handleNav('/dashboard')} className={`relative group flex items-center gap-1.5 px-3 py-2 font-sans font-black text-xs uppercase tracking-widest transition-all duration-300 ${path === '/dashboard' ? 'text-[#8B1A2F] scale-105' : 'text-[#2B050C] hover:text-[#8B1A2F] hover:scale-105'}`}>
                      <Home size={14} className="stroke-[2.5px] transition-transform group-hover:scale-110" />
                      <span>Home</span>
                      {path === '/dashboard' && <div className="absolute bottom-[-4px] left-3 right-3 h-[2px] bg-[#8B1A2F] rounded-full" />}
                    </button>
                    <button onClick={() => handleNav('/about')} className={`relative group flex items-center gap-1.5 px-3 py-2 font-sans font-black text-xs uppercase tracking-widest transition-all duration-300 ${path === '/about' ? 'text-[#8B1A2F] scale-105' : 'text-[#2B050C] hover:text-[#8B1A2F] hover:scale-105'}`}>
                      <Info size={14} className="stroke-[2.5px] transition-transform group-hover:scale-110" />
                      <span>About</span>
                      {path === '/about' && <div className="absolute bottom-[4px] left-3 right-3 h-[2px] bg-[#8B1A2F] rounded-full" />}
                    </button>
                    <button onClick={() => handleNav('/maya')} className={`relative group flex items-center gap-1.5 px-3 py-2 font-sans font-black text-xs uppercase tracking-widest transition-all duration-300 ${path === '/maya' ? 'text-[#9F7E39] scale-105' : 'text-[#7A5F24] hover:text-[#9F7E39] hover:scale-105'}`}>
                      <MessageCircle size={14} className="stroke-[2.5px] transition-transform group-hover:scale-110" />
                      <span>Maya</span>
                      {path === '/maya' && <div className="absolute bottom-[4px] left-3 right-3 h-[2px] bg-[#9F7E39] rounded-full" />}
                    </button>
                    <button onClick={() => handleNav('/match')} className={`relative group flex items-center gap-1.5 px-3 py-2 font-sans font-black text-xs uppercase tracking-widest transition-all duration-300 ${path === '/match' ? 'text-[#8B1A2F] scale-105' : 'text-[#2B050C] hover:text-[#8B1A2F] hover:scale-105'}`}>
                      <Sparkles size={14} className="stroke-[2.5px] transition-transform group-hover:scale-110" />
                      <span>Match</span>
                      {path === '/match' && <div className="absolute bottom-[-4px] left-3 right-3 h-[2px] bg-[#8B1A2F] rounded-full" />}
                    </button>
                  </div>
                  <div className="h-8 w-[2px] bg-brand-primary/20 hidden md:block mx-1 sm:mx-2"></div>
                  <button onClick={() => handleNav('/profile')} className="group relative flex items-center gap-3 px-3 py-2 cred-elevation hover:bg-brand-primary/5 transition-all border-brand-primary/10 text-left rounded-xl">
                    ...
                    <div className="hidden lg:block text-left mr-2 font-display">
                      <p className="text-[10px] font-black uppercase text-text-base leading-tight truncate max-w-[100px] font-display">{profile?.displayName?.split(' ')[0] || 'User'}</p>
                      <p className="text-[8px] font-bold uppercase text-brand-accent tracking-widest leading-tight font-display">{profile?.isPremium ? 'PRO member' : 'Basic Member'}</p>
                    </div>
                  </button>
                  <button onClick={() => setShowLogoutConfirm(true)} className="group relative btn-logout-round w-10 h-10 lg:w-12 lg:h-12 flex items-center justify-center text-text-muted hover:text-text-base">
                    <LogOut size={18} className="sm:size-[20px]" />
                    <span className="absolute -bottom-10 left-1/2 -translate-x-1/2 px-3 py-1 bg-brand-primary text-white text-[10px] font-black uppercase tracking-widest rounded opacity-0 lg:group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50 shadow-[0_4px_10px_rgba(128,0,32,0.15)] bg-slate-800">Logout</span>
                  </button>
                </div>
              ) : showPublicNav ? (
                <div className="flex items-center gap-2 sm:gap-4">
                  <button onClick={() => handleNav('/about')} className={`btn-glassy group flex items-center justify-center gap-2 hover:text-[#9F7E39] ${path === '/about' ? 'btn-glassy-active' : 'text-[#5A1E2D]'}`}>
                    <span>About</span>
                  </button>
                  <button onClick={() => handleNav('/auth')} className="btn-get-started !py-2.5 !px-4.5 !text-[10px] sm:!text-[11px]">
      ...
      {showNav && (
        <div className="md:hidden fixed bottom-6 left-4 right-4 py-2.5 px-4 flex justify-between items-center z-50 rounded-full bg-[#F8F4EE]/90 backdrop-blur-xl border border-[#5A1E2D]/15 shadow-[0_12px_30px_rgba(90,30,45,0.12)] animate-in fade-in slide-in-from-bottom-5 duration-300">
          <button onClick={() => handleNav('/dashboard')} className={`transition-all duration-300 flex flex-col items-center justify-center gap-1 px-3 py-1.5 rounded-full ${path === '/dashboard' ? 'text-[#8B1A2F]' : 'text-[#2B050C] hover:text-[#8B1A2F] active:scale-95'}`}>
          ...
          <button onClick={() => handleNav('/maya')} className={`transition-all duration-300 flex flex-col items-center justify-center gap-1 px-3 py-1.5 rounded-full ${path === '/maya' ? 'text-[#9F7E39]' : 'text-[#7A5F24] hover:text-[#9F7E39] active:scale-95'}`}>
          ...
          <button onClick={() => handleNav('/match')} className={`transition-all duration-300 flex flex-col items-center justify-center gap-1 px-3 py-1.5 rounded-full ${path === '/match' ? 'text-[#8B1A2F]' : 'text-[#2B050C] hover:text-[#8B1A2F] active:scale-95'}`}>
          ...
          <button onClick={() => handleNav('/profile')} className={`transition-all duration-300 flex flex-col items-center justify-center gap-1 px-3 py-1.5 rounded-full ${path === '/profile' ? 'text-[#8B1A2F]' : 'text-[#2B050C] hover:text-[#8B1A2F] active:scale-95'}`}>
          ...
          <button onClick={() => handleNav('/about')} className={`transition-all duration-300 flex flex-col items-center justify-center gap-1 px-3 py-1.5 rounded-full ${path === '/about' ? 'text-[#8B1A2F]' : 'text-[#2B050C] hover:text-[#8B1A2F] active:scale-95'}`}>
        ...
        {incomingMessageToast && (
          ...
          >
            <div className="flex-1 space-y-1.5 text-left cursor-pointer font-sans" onClick={() => {
              ...
              if (notifType === 'request') targetTo = '/dashboard';
              else if (notifType === 'reject') targetTo = '/match';
              else if (toConnId) targetTo = `/chat/${toConnId}`;
              else targetTo = '/dashboard';
              if (triggerBordeaux) {
              ...
              <div className="flex items-center gap-2">
                ...
                <p className="text-[9px] font-sans font-black uppercase tracking-widest text-brand-accent font-display">
                  {incomingMessageToast.type === 'request' ? 'Friend Request' :
                   ...
                   incomingMessageToast.type === 'reject' ? 'Request Update' : 'New Message'}
                </p>
              </div>
              <h5 className="text-base font-display font-black uppercase italic leading-none text-text-base">{incomingMessageToast.senderName}</h5>
              <p className="font-sans font-light text-xs text-text-muted pr-6 block max-w-[280px] leading-relaxed break-words">{incomingMessageToast.text}</p>
            ...
            <button onClick={() => setIncomingMessageToast(null)} className="text-text-muted hover:text-brand-primary transition-colors p-1">
              <X size={14} />
            </button>
          </motion.div>
        )}
          ...
          <div className="fixed inset-0 bg-bg-base/70 backdrop-blur-md z-[500] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-2">
                <h3 className="font-display text-xl font-black uppercase italic text-text-base">A Temporary Pause?</h3>
                <p className="font-sans text-[10px] text-text-muted leading-relaxed uppercase tracking-wider">Are you sure you want to end your current deep connection session?</p>
              </div>
              <div className="flex gap-4 pt-1">
                <button type="button" onClick={() => setShowLogoutConfirm(false)} className="btn-secondary flex-1 py-3 text-[10px] font-black uppercase tracking-widest text-text-muted hover:text-text-base hover:bg-brand-primary/5 hover:border-brand-primary/30 transition-all border border-brand-primary/10 rounded-xl">No, Stay</button>
                <button type="button" onClick={() => { setShowLogoutConfirm(false); auth.signOut(); }} className="btn-primary flex-1 py-3 bg-brand-primary hover:bg-[#A3002E] text-white text-[10px] font-black uppercase tracking-widest transition-all rounded-xl">Yes, Log Out</button>
              </div>
            </motion.div>
          </div>
        )}
...
const routeTree = rootRoute.addChildren([
  indexRoute, authRoute, onboardingRoute, dashboardRoute,
  mayaRoute, matchRoute, aboutRoute, chatRoute,
  profileRoute, privacyRoute, termsRoute, protocolRoute
]);

const router = createRouter({ routeTree });

// ✅ FIXED: /api/notify replaced with /api/send-welcome-email
const sendWelcomeEmailWithResend = async (userEmail: string, userName: string) => {
  try {
    console.log("[App.tsx] Sending welcome email to:", userEmail, "Name:", userName);
    ...
    console.log("[App.tsx] Welcome email API response:", data);
    return response.ok;
  } catch (err) {
    console.error("[App.tsx] Failed to send welcome email:", err);
    return false;
  }
};
...
const sendLoginEmailWithResend = async (userEmail: string, userName: string) => {
  try {
    console.log("[App.tsx] Sending login alert to:", userEmail, "Name:", userName);
    const response = await fetch('/api/send-welcome-email', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    const data = await response.json();
    console.log("[App.tsx] Login email API response:", data);
    return response.ok;
  } catch (err) {
    console.error("[App.tsx] Failed to send login email:", err);
    return false;
  }
};
...
export default function App() {
    ...
    const initProfile = async () => {
      ...
      if (localSaved) {
        try {
          ...
          if (parsed && parsed.onboarded) setProfile(parsed);
        } catch (_) {}
      }
      ...
      try {
        ...
        if (!displayNameToUse && isNewProfile) {
          ...
          try {
            ...
            if (reloadedUser) displayNameToUse = reloadedUser.displayName || '';
          } catch (e) {
        ...
        if (shouldSendWelcome && emailToNotify) {
          sendWelcomeEmailWithResend(emailToNotify, nameToNotify).then(async (success) => {
            if (success) {
              console.log("[App.tsx] Welcome email sent. Saving welcomeEmailSent: true to Firestore.");
              await updateDoc(profileRef, { welcomeEmailSent: true });
            } else {
              console.warn("[App.tsx] Welcome email failed. Will retry on next reload.");
            ...
            if (isNewDay) {
              ...
              if (!localStorage.getItem(RESET_KEY)) {
                ...
                setDoc(doc(db, 'conversations_maya', user.uid), {
                  ...
                  console.error("Maya chat reset failed:", err);
                });
              }
            }
  ...
  useEffect(() => {
    ...
    const updatePresence = async () => {
      try {
        ...
        console.warn("Presence update ignored:", e);
      }
    };
    updatePresence();
    const interval = setInterval(async () => {
      if (auth.currentUser && !document.hidden) await updatePresence();
    }, 60000);
  ...
  const triggerBordeaux = React.useCallback((onMeetCenter?: () => void, onComplete?: () => void, isOnboardingTransition?: boolean) => {
    ...
    setBordeauxConfig({ show: true, onMeetCenter, onComplete });
  }, []);
  ...
  return (
    <AuthContext.Provider value={{ user, profile, setProfile, loading, theme, toggleTheme, connectionError, triggerBordeaux }}>
      ...
      {bordeauxConfig.show && (
        <BordeauxTransition
          onMeetCenter={() => { if (bordeauxConfig.onMeetCenter) bordeauxConfig.onMeetCenter(); }}
          onComplete={() => {
            setBordeauxConfig(prev => ({ ...prev, show: false }));
            if (bordeauxConfig.onComplete) bordeauxConfig.onComplete();
          }}
        />
      )}
