...
// Restore Resend Email Notification Dispatch helper
const sendWelcomeEmailWithResend = async (userEmail: string, userName: string) => {
  try {
    console.log("[App.tsx] Sending welcome email via Resend API to:", userEmail, "Name:", userName);
    ...
    console.error("[App.tsx] Failed to send welcome email via Resend API:", err);
    return false;
  }
};
  ...
  useEffect(() => {
    ...
    const initProfile = async () => {
      ...
      try {
        ...
        // Dispatch Welcome Email via Resend API
        if (shouldSendWelcome && emailToNotify) {
          sendWelcomeEmailWithResend(emailToNotify, nameToNotify).then(async (success) => {
            if (success) {
              ...
              console.warn("[App.tsx] Welcome email dispatch failed. Will retry on next session.");
              sessionStorage.removeItem(welcomeEmailSessionKey);
            }
          }).catch(err => {
            console.error("[App.tsx] Welcome email dispatch exception:", err);
            sessionStorage.removeItem(welcomeEmailSessionKey);
          });
        }
